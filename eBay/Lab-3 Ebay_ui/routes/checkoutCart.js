						...
						for(var j=0; j < availableItems.length; j++){
							...
							if(cartItems[i].itemId === availableItems[j].itemId && cartItems[i].qty <= availableItems[j].qtyLeft){
								...
								var params = {
											...
											userEmail: cartItems[i].userEmail, 
											qty: cartItems[i].qty, 
								...
								soap.createClient(baseURL,option, function(err, client) {
								      if(err){
								    	  ...
								    	  client.checkOutCart(params, function(err, result) {
								    		  console.log("In checkOutCart Method");
									    	  if(err){
									    		  console.log("Error in SOAP checkOutCart" + err);
									    	  }
									      }); 
								      }
								  });
