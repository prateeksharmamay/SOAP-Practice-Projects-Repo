...
var json_responses, sessionStatus, sessionEmail;
	...
	if(req.session.email){
		...
		var params = {bidUser: sessionEmail, itemId: itemId, bidAmount: bidAmount};
		console.log("params: " + params);
		soap.createClient(baseURL,option, function(err, client) {
		      if(err){
		    	  console.log("Error in SOAP Request" + err);
		      }else{
		    	  client.addBidToItem(params, function(err, result) {
		    		  console.log("In addBidToItem Method");
			    	  if(err){
			    		  console.log("Error in SOAP addBidToItem" + err);
			    	  }else{
