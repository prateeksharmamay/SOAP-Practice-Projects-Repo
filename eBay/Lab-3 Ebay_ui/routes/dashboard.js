...
exports.addItemToUserCart = function(req,res){
	...
	if(req.session.email){
		...
		var params = {buyerEmail:email, itemId:item_id};
		console.log("params: " + params);
		soap.createClient(baseURL,option, function(err, client) {
		      if(err){
		    	  console.log("Error in SOAP Request" + err);
		      }else{
		    	  client.addItemToUserCart(params, function(err, result) {
		    		  console.log("In addItemToUserCart Method");
			    	  if(err){
			    		  console.log("Error in SOAP addItemToUserCart" + err);
			    	  }else{
			    		  	console.log("User Cart Data Inserted");
